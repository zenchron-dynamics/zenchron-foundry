...
for ref in $mutated; do
  ...
  if [ "$prior" = "NONE" ]; then
    ...
    if reg_digest "$ref" >/dev/null 2>&1; then echo "STILL-PRESENT $ref (expected removed)" >> "$INCIDENT.tmp"; bad=$((bad+1)); else restored=$((restored+1)); fi
  else
...
echo "rollback: ${restored} restored, ${bad} failed"
if [ "$bad" -gt 0 ]; then
  {
...
rm -f "$INCIDENT.tmp"
echo "ROLLBACK OK: all mutated aliases restored"
