...
# The CURRENT governance evidence. Older dated records (2026-08-02, 2026-08-13)
# are historical: content_binding pins the governed inputs by digest, so any
# legitimate change to one of them invalidates every earlier record by design.
# That is the control working, not drift — but it means this pointer moves to
# the newest record whenever a bound input changes, and the record is
# regenerated by `verify-repo-governance.sh --evidence <path>` against the LIVE
# control plane, never hand-edited.
EVIDENCE=docs/audits/governance-verification-2026-08-24.json
...
# DERIVED from the policy, never pinned. This asserted `== 5`, the second copy
# of the same hardcoded count that verify-repo-governance.sh carried, so
# declaring a check the ruleset already required broke it too. The invariant is
# that the evidence records the two sets DISTINCTLY and agrees with the policy —
# not that either has some particular size.
ck "evidence records BOTH check sets, distinctly" \
   "python3 -c \"
import json, yaml
d = json.load(open('$EVIDENCE'))
pol = yaml.safe_load(open('policies/required-release-checks.yaml'))
assert d['pr_required_checks'] == pol['pr_required_checks'], (d['pr_required_checks'], pol['pr_required_checks'])
assert d['release_required_checks'] == pol['release_required_checks']
...
ck "...and neither check-set size is pinned as a literal in the suite" \
   "! grep -qE \"len\\(d\\['pr_required_checks'\\]\\)==[0-9]+\" '$0'"
...
# THIS ASSERTION WAS VACUOUS AND SAID SO IN CAPITALS.
#
# It ran the guard through `bash -c` and asserted the command failed. The
# extraction did not define the function, so `bash -c` exited 127 — command not
# found — and the leading `!` turned that into a PASS. It reported "the helper
# REFUSES the exact payload" while proving only that a name was unbound. Every
# run since the 2026-08-06 rewrite passed for that reason.
#
# Once the function is actually defined, the claim turns out to be FALSE, and
# correctly so: since that rewrite the payload the incident used is a SAFE
# payload. GitHub 422s `selected_repository_ids`, so the helper must omit it, and
# omitting it is what clears membership. The payload shape is no longer the
# control. The control is the post-PATCH verification and restore, which the two
# assertions below this one check.
#
# So the guard is asserted on what it really enforces — the forbidden keys — and
# the committed incident payload is asserted to be ACCEPTED, which is the honest
# statement and the reason the restore step has to exist.
_payload_guard() { # _payload_guard <payload.json> -> guard's own exit status
  bash -c '
    ...
    command -v assert_patch_payload_safe >/dev/null || exit 3
    die() { printf "REFUSE: %s\n" "$*" >&2; return 1; }
    assert_patch_payload_safe "$1"' _ "$1" >/dev/null 2>&1
}
GT="$(mktemp -d)"
INCIDENT=docs/audits/runner-group-swap-2026-08-02/patch-payload.json
ck "the payload guard is actually DEFINED (127 must never read as a refusal)" \
   "_payload_guard '$INCIDENT'; [ \"\$?\" != 3 ]"
for _k in selected_repository_ids runners repositories; do
  python3 -c "
import json,sys
p=json.load(open('$INCIDENT')); p[sys.argv[1]]=[1]
json.dump(p, open('$GT/'+sys.argv[1]+'.json','w'))" "$_k"
  ck "the guard REFUSES a group PATCH carrying '$_k'" \
     "! _payload_guard '$GT/$_k.json'"
done
ck "the committed incident payload is ACCEPTED by the guard — which is WHY the restore step exists" \
   "_payload_guard '$INCIDENT'"
rm -rf "$GT"
